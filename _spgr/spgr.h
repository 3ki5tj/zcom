...
/* Copyright (C) 2010 Atsushi Togo spglib-1.3/spg_database.c(.h)
 * adapted by CZ for ZEFSA II */
...
static const SpacegroupType spacegroup_types[] = {
  ...
  {200, "Th^1  ", "-P 2 2 3        ", "P m -3                         ", "P 2/m -3           ", "Pm-3      ", CUBIC }, /*494*/
  {201, "Th^2  ", "P 2 2 3 -1n     ", "P n -3                         ", "P 2/n -3           ", "Pn-3      ", CUBIC }, /*495*/
  {201, "Th^2  ", "-P 2ab 2bc 3    ", "P n -3                         ", "P 2/n -3           ", "Pn-3      ", CUBIC }, /*496*/
  {202, "Th^3  ", "-F 2 2 3        ", "F m -3                         ", "F 2/m -3           ", "Fm-3      ", CUBIC }, /*497*/
  {203, "Th^4  ", "F 2 2 3 -1d     ", "F d -3                         ", "F 2/d -3           ", "Fd-3      ", CUBIC }, /*498*/
  {203, "Th^4  ", "-F 2uv 2vw 3    ", "F d -3                         ", "F 2/d -3           ", "Fd-3      ", CUBIC }, /*499*/
  {204, "Th^5  ", "-I 2 2 3        ", "I m -3                         ", "I 2/m -3           ", "Im-3      ", CUBIC }, /*500*/
  {205, "Th^6  ", "-P 2ac 2ab 3    ", "P a -3                         ", "P 2_1/a -3         ", "Pa-3      ", CUBIC }, /*501*/
  {206, "Th^7  ", "-I 2b 2c 3      ", "I a -3                         ", "I 2_1/a -3         ", "Ia-3      ", CUBIC }, /*502*/
...
/* get all rotation matrices and translation vectors
 * return # of operations */
int spgr_getmat(int hall, int rot[][3][3], double trans[][3])
{
  int i, ist, cnt;
  ...
  for (i = 0; i < cnt; i++)
    spgr_getop(rot[i], trans[i], ist + i);
  return cnt;
}

/* check it has inversion */
int spgr_iscsym(int hall)
{
  int i, ist, cnt;
  ...
  ist = symmetry_operation_index[ hall ][1];
  for (i = 0; i < cnt; i++)
    if (symmetry_operations[ist + i] == 3198)
      return 1;
  return 0;
}

/* compare two strings, `t' may have trailing white spaces */
int strspcmp(const char *s, const char *t)
{
  for (; *s && *t; s++, t++) {
    if (*s > *t) return 1;
    else if (*s < *t) return -1;
  }
  if (*s == '\0' && isspace(*t)) return 0;
  return (*s == *t) ? 0 : (*s) - (*t);
}

void rmsp(char *s, const char *t)
{
  for (; *t; t++)
    if (!isspace(*t)) *s++ = *t;
  *s = '\0';
 ...
 * from the international code `s', such as P4_122 */
int spgr_getmatstr(const char *s, int *hall, int rot[][3][3], double trans[][3])
{
  const int SPMAX = 530;
  int i;
  ...
  for (i = 1; i <= SPMAX; i++) {
    rmsp(buf1, spacegroup_types[i].international_full);
    rmsp(buf2, spacegroup_types[i].international);

    if (0 == strspcmp(s, buf2)
     || 0 == strspcmp(s, buf1)
     || 0 == strspcmp(s, spacegroup_types[i].international_short))
      break;
  }
  if (i > SPMAX) {
  ...
  *hall = i;
  return spgr_getmat(i, rot, trans);
}

/* get number */
int spgr_getnum(int hall, int cs)
{
  return spacegroup_types[hall].number;
}

/* get point group name */
char *spgr_getpgname(int hall, int cs)
{
  int i = spacegroup_types[hall].holohedry;
  char *p;
  static char s[32];

  strcpy(s, Holohedry_name[i]);
  if (cs > 0) for (p = s; *p; p++) *p = toupper(*p);
  if (cs < 0) for (p = s; *p; p++) *p = tolower(*p);
  return s;
}
