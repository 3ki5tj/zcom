...
gamma = 3.0;
F(rho) = exp(-gamma*rho*rho);
G1(rho) = (1 - F(rho))/(2*gamma);
G2(rho) = -(F(rho)*rho**2 - 2*G1(rho))/(2*gamma);
G3(rho) = -(F(rho)*rho**4 - 4*G2(rho))/(2*gamma);
G4(rho) = -(F(rho)*rho**6 - 6*G3(rho))/(2*gamma);
G5(rho) = -(F(rho)*rho**8 - 8*G4(rho))/(2*gamma);
G6(rho) = -(F(rho)*rho**10 - 10*G5(rho))/(2*gamma);

# the potential part of the free energy of the Lennard-Jones fluid
Ar(rho, T) = rho*(a1(T) + rho*(a2(T)/2 + rho*(a3(T)/3 + rho*(a4(T)/4 + rho*(a5(T)/5 + rho*(a6(T)/6 + rho*(a7(T)/7 + rho*a8(T)/8))))))) + b1(T)*G1(rho) + b2(T)*G2(rho) + b3(T)*G3(rho) + b4(T)*G4(rho) + b5(T)*G5(rho) + b6(T)*G6(rho);
